...
#
#  This file should be source("./TilleyK2_07102017.R") during the R-session.
##########################################################################################

TilleyK2.072017 <- function(V,x,T,dt,allstates=FALSE,returnNDX=6,gating=FALSE){
...
runIV <- function(Vstep,Vhold=-100,tstep=c(50,100,200),prefix="k2IV-"){
    x <- initX(1)
    for (i in seq(1,length(Vstep))){
        V <- c(Vhold,Vstep[i],Vhold)
        O <- TilleyK2.072017(V,x,T=tstep,dt=0.1,allstates=FALSE,gating=TRUE)
...
runAct <- function(Vstep,Vhold=-100,tstep=c(50,100),prefix="k2Act-"){
    x <- initX(1)
    for (i in seq(1,length(Vstep))){
        V <- c(Vhold,Vstep[i])
        O <- TilleyK2.072017(V,x,T=tstep,dt=0.1,allstates=FALSE,gating=TRUE)
